...
public class LoginView extends JFrame {
    ...
    private void abrirTelaCadastro() {
        this.dispose();
        new CadastroVistoriadorView().setVisible(true);
    }
    ...
    private void abrirTelaAposLogin(String cpf, String perfil) {
        ...
        try {
            switch (perfil.toUpperCase()) {
                case "PROPRIETÁRIO":
                    Proprietario proprietario = PessoaController.buscarProprietarioPorCpf(cpf);
                    new ProprietarioView(new AgendamentoController(), proprietario).setVisible(true);
                    break;
                case "VISTORIADOR":
                    Vistoriador vistoriador = PessoaController.buscarVistoriadorPorCpf(cpf);
                    new VistoriadorAgendamentosView(vistoriador).setVisible(true);
                    break;
                case "ATENDENTE":
                    new PrincipalAtendenteView().setVisible(true);
                    break;
            }
        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(null, "Erro ao buscar dados do usuário: " + ex.getMessage(), 
