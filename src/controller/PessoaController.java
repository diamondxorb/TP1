...
public class PessoaController {
    ...
    public static Vistoriador buscarVistoriadorPorCpf(String cpf) throws SQLException {
        String sql = "SELECT * FROM pessoa WHERE tipo = 'Vistoriador' AND cpf = ?";
        try (Connection conn = Conexao.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, cpf);
            ResultSet rs = stmt.executeQuery();
            if (rs.next()) {
                return new Vistoriador(
                    rs.getInt("id"),
                    rs.getString("nome"),
                    rs.getString("cpf"),
                    rs.getString("data_nascimento"),
    ...
    public static Proprietario buscarProprietarioPorCpf(String cpf) throws SQLException {
        String sql = "SELECT * FROM pessoa WHERE tipo = 'Proprietário' AND cpf = ?";
        try (Connection conn = Conexao.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, cpf);
            ResultSet rs = stmt.executeQuery();
